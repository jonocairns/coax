...
- M0b includes only the minimum Xtream vertical slice needed to exercise real channels: credentials seeded from ignored local development input into safe storage, account validation, live categories/streams, normalized internal channel records and stream URL resolution. M1 adds a minimal first-run Xtream setup flow and the v1 M3U adapter so the recovery engine can be daily-driven without hand-edited configuration. Polished management of multiple configured sources remains M3: it uses opaque source IDs, safe renderer-visible summaries, separate encrypted credential records, and one explicitly selected active source rather than channel aggregation.
...
- **M1 — Engine (the long pole):** minimal in-app Xtream setup, the v1 M3U adapter, generation-safe supervisor, bounded transport-aware recovery, two-phase buffer model, full fault-injection harness, data utility-process boundary, and full §7.1 taxonomy green. UI remains deliberately minimal. Daily-drivable on engine merit without hand-edited source configuration. Slice-level execution and evidence requirements are defined in [`docs/M1_IMPLEMENTATION.md`](docs/M1_IMPLEMENTATION.md).
...
- **M3 — TiviMate parity (personal):** EPG grid, polished Xtream login/source management with multiple configured sources and one active-source selector, favourites/groups, VSR toggle, settings and first-run credential flow.
